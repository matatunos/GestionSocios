...
class EventController {
    ...
    public function markPaid($eventId, $memberId) {
        ...
        // Obtener datos del socio para descripción detallada en contabilidad
        $memberStmt = $this->db->prepare("SELECT first_name, last_name FROM members WHERE id = ?");
        $memberStmt->execute([$memberId]);
        $member = $memberStmt->fetch(PDO::FETCH_ASSOC);
        $memberFullName = $member ? trim($member['first_name'] . ' ' . $member['last_name']) : "Socio #$memberId";
        $accountingDescription = "Evento: {$this->event->name} - Socio #$memberId: $memberFullName";
