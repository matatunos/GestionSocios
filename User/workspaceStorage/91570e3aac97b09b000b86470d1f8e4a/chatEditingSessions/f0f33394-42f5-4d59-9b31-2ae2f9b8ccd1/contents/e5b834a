    ...
    public function store() {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if ($this->donation->create()) {
                ...
                // Obtener datos del donante para descripción detallada
                $donorStmt = $this->db->prepare("SELECT name FROM donors WHERE id = ?");
                $donorStmt->execute([$_POST['donor_id']]);
                $donor = $donorStmt->fetch(PDO::FETCH_ASSOC);
                $donorName = $donor ? $donor['name'] : "Donante #" . $_POST['donor_id'];
                
                $donationType = $_POST['type'] ?? 'monetary';
                $description = "Donación $donationType ({$_POST['year']}) - $donorName";
